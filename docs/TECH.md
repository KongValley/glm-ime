...
| B12 | **测试宿主破坏性**（抢前台/进程风暴）| e2e_host 依赖前台且流程粗暴 | 温和 3 次尝试+失败即报 ENV_UNAVAILABLE；禁止动桌面/无关进程 | ✅ | 忙时实测自动报告不干扰；空闲 13/13 |
...
| B17 | 任务栏一排 glm 窗口 | launcher/引擎为控制台程序且引擎 spawn 未禁窗口 | launcher `#![windows_subsystem]` + 引擎 `CREATE_NO_WINDOW` + 启动孤儿清理 | ✅ | 部署后窗口数 0（实测）|
| B18 | 引擎/会话行为无日志可查（8 引擎残留无从追溯）| 日志覆盖不全 | 全事件日志体系（launcher 会话含 client_pid；引擎文件日志+PANIC hook；TIP 事件）| ✅ | 冒烟日志实测（§5 日志策略）|
...
### 日志策略（全事件黑匣子，2026-09-26 按用户要求建立）

**原则：程序的每一步（生命周期/状态变迁/错误）都写日志；按键/重绘等热路径用累计计数，会话结束汇总——保可追溯且零性能损耗。**

| 组件 | 文件 | 必记事件 |
|---|---|---|
| launcher | `%LOCALAPPDATA%\glm-ime\logs\launcher.log` | 启动/孤儿清理；会话 start（client_pid）/每个引擎 spawn（pid）/respawn（原因+计数）/session end（请求数+respawn 数+kill 结果+时长）；connect/create/pipe 错误 |
| engine-rs | `Engine-<pid>.log` | 启动/init（词库统计）/bad request/**PANIC 全量**/退出 |
| engine-py | `Engine-py-<pid>.log` | 启动/init（词库统计）/异常完整 traceback |
| TIP | `TipLog-<pid>.log` | 激活/去激活/候选窗创建/引擎 init 失败/SEH/连接与 IO 错误 |

- 违规检查：**热路径（每次按键/每次重绘）禁止写文件**；临时探针用完即删（提交前检查）。
- 排障首选：先读上述日志定位到组件与事件，再看 WER→map 定位代码。
...
- [x] **C1**：卡顿修复变更集（B6/B7/B8）已回归并提交
- [x] **C2**：B12 测试非侵入化完成（温和前台 + ENV_UNAVAILABLE 明确报告）
- [x] **C3**：用户确认已不卡顿
- [x] **C4**：控制台窗口泄漏（B17）修复 + 全事件日志体系（B18）
- [ ] M6 路线图项（§7）
